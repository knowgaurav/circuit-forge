...
export function MiniCanvas({ blueprint, width = 500, height = 250 }: MiniCanvasProps) {
    ...
    useEffect(() => {
        const { components: loadedComponents, wires: loadedWires } =
            loadCircuitFromBlueprint(blueprint);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            setComponents((prev) => {
                ...
                const isRisingEdge = prev.some(
                    (c) => c.type === 'CLOCK' && ((c.properties.phase as number) ?? 0) % 2 === 0
                );
                ...
                return prev.map((c) => {
                    if (c.type === 'CLOCK') {
                        return {
                            ...
                            properties: {
                                ...c.properties,
                                phase: ((c.properties.phase as number) ?? 0) + 1,
                            },
                        };
                    }
                    ...
                    if (c.type === 'SHIFT_REGISTER_8BIT' && isRisingEdge) {
                        ...
                        return {
                            ...c,
                            properties: { ...c.properties, _shiftValue: (shiftValue << 1) % 256 },
                        };
                    }
                    return c;
                });
                    ...
                    if (comp.type === 'SWITCH_TOGGLE' || comp.type === 'SWITCH_PUSH') {
                        ...
                        setComponents((prev) => {
                            ...
                            if (comp.type === 'SWITCH_PUSH' && wasOff) {
                                ...
                                if (clockWire) {
                                    const targetComp = updated.find(
                                        (c) => c.id === clockWire.toComponentId
                                    );
                                    const targetPin = targetComp?.pins.find(
                                        (p) => p.id === clockWire.toPinId
                                    );
                                        ...
                                        return updated.map((c) => {
                                            if (c.id === targetComp.id) {
                                                ...
                                                if (c.type === 'COUNTER_4BIT') {
                                                    const count =
                                                        (c.properties._count as number) ?? 0;
                                                    return {
                                                        ...c,
                                                        properties: {
                                                            ...c.properties,
                                                ...
                                                if (c.type === 'SHIFT_REGISTER_8BIT') {
                                                    const shiftValue =
                                                        (c.properties._shiftValue as number) ?? 0;
                                                    // Find what's connected to SI pin
                                                    const siPin = c.pins.find(
                                                        (p) => p.name === 'SI'
                                                    );
                                                    const siWire =
                                                        siPin &&
                                                    ...
                                                    if (siWire) {
                                                        ...
                                                        const srcComp = updated.find(
                                                            (src) =>
                                                                src.id === siWire.fromComponentId
                                                        );
                                                        if (
                                                            srcComp &&
                                                            (srcComp.type === 'SWITCH_TOGGLE' ||
                                                                srcComp.type === 'SWITCH_PUSH')
                                                        ) {
                                                            dataBit = srcComp.properties.state
                                                                ? 1
                                                                : 0;
                                                        }
                                                    }
    ...
    const handleMouseUp = useCallback(() => {
        setComponents((prev) =>
            prev.map((c) =>
                c.type === 'SWITCH_PUSH'
                    ? { ...c, properties: { ...c.properties, state: false } }
                    : c
            )
        );
    }, []);
