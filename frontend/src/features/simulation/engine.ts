...
const SOURCE = new Set<ComponentType>([
    'CONST_HIGH', 'CONST_LOW', 'VCC_5V', 'VCC_3V3', 'GROUND', 'SWITCH_TOGGLE', 'SWITCH_PUSH',
]);
const STATEFUL = new Set<ComponentType>([
    'SR_LATCH', 'D_FLIPFLOP', 'JK_FLIPFLOP', 'T_FLIPFLOP',
    'COUNTER_4BIT', 'SHIFT_REGISTER_8BIT', 'CLOCK',
]);
...
export class SimulationEngine {
    ...
    loadCircuit(circuit: CircuitState): void {
        ...
        this.driver = new Map(this.wires.map((w) =>
            [`${w.toComponentId}:${w.toPinId}`, `${w.fromComponentId}:${w.fromPinId}`]));
        this.run();
    }
    ...
    setInput(componentId: string, value: boolean): void {
        ...
        else if (c.type === 'SWITCH_PUSH') { props.pressed = value; props.state = value; }
        this.run();
    }
    ...
    private refreshSources(): void {
        this.components.forEach((c, cid) => {
            ...
            if (t === 'CONST_HIGH' || t === 'VCC_5V' || t === 'VCC_3V3') this.publish(cid, { OUT: H });
            else if (t === 'CONST_LOW' || t === 'GROUND') this.publish(cid, { OUT: L });
            else if (t === 'SWITCH_TOGGLE') this.publish(cid, { OUT: pr.state ? H : L });
            else if (t === 'SWITCH_PUSH') this.publish(cid, { OUT: (pr.pressed || pr.state) ? H : L });
            else if (t === 'CLOCK') {
    ...
    private topo(): { order: string[]; inCycle: string[] } {
        ...
        comb.forEach((cid) => { adj.set(cid, []); indeg.set(cid, 0); });
        for (const w of this.wires) {
        ...
        comb.forEach((cid) => { if (!ordered.has(cid)) inCycle.push(cid); });
    ...
    private tick(c: CircuitComponent, inp: Record<string, Signal>, st: ComponentState): void {
        const t = c.type;
        if (t === 'SR_LATCH') {
            const s = inp.S ?? L; const r = inp.R ?? L;
            st.internal.Q = s === H && r === H ? X : s === H ? H : r === H ? L : ((st.internal.Q as Signal) ?? L);
            return;
        }
        const clk = inp.CLK ?? L;
        ...
        if (t === 'D_FLIPFLOP') st.internal.Q = inp.D ?? L;
            ...
            const j = inp.J ?? L; const k = inp.K ?? L;
    ...
    private computeOutputs(comp: CircuitComponent, inputs: Record<string, Signal>,
        _state: ComponentState): Record<string, Signal> {
        const t = comp.type;
        const vs = comp.pins.filter((p: Pin) => p.type === 'input').map((p) => inputs[p.id] ?? X);
        const a = inputs.A ?? X; const b = inputs.B ?? X;
        ...
        if (t === 'ADDER_4BIT') {
            const ai = bits(inputs, 'A', 4); const bi = bits(inputs, 'B', 4);
        ...
        if (t === 'BCD_TO_7SEG') {
            ...
            const segs = SEG[n % 10] ?? '';
            const r: Record<string, Signal> = {};
            for (const p of comp.pins) if (p.type === 'output') r[p.id] = segs.includes(p.id.toUpperCase()) ? H : L;
            return r;
        }
        const r: Record<string, Signal> = {};
