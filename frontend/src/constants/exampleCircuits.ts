...
function resolvePin(
    ...
    if (!compType) {
        throw new Error(
            `Unknown component label: "${label}". Available: ${Array.from(componentMap.keys()).join(', ')}`
        );
    }
    ...
    if (pinName) {
        ...
        const pin = pins.find((p) => p.name && p.name.toLowerCase() === pinName.toLowerCase());
        if (!pin) {
            const available = pins.map((p) => p.name || 'unknown').join(', ');
            throw new Error(
                `${direction} pin "${pinName}" not found on ${compType} (${label}). Available: ${available}`
            );
        }
...
export const OR_GATE_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_TOGGLE', label: 'SW1', position: pos(80, 70) },
            { type: 'SWITCH_TOGGLE', label: 'SW2', position: pos(80, 130) },
            { type: 'OR_2', label: 'OR1', position: pos(200, 100) },
            { type: 'LED_RED', label: 'LED1', position: pos(320, 100) },
        ],
        [
            ['SW1', 'OR1:A'],
            ['SW2', 'OR1:B'],
...
export const AND_GATE_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            ...
            { type: 'LED_GREEN', label: 'LED1', position: pos(320, 100) },
        ],
        [
            ['SW1', 'AND1:A'],
            ['SW2', 'AND1:B'],
...
export const NOT_GATE_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            ...
            { type: 'NOT', label: 'NOT1', position: pos(200, 100) },
...
export const NAND_GATE_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_TOGGLE', label: 'SW1', position: pos(80, 70) },
            { type: 'SWITCH_TOGGLE', label: 'SW2', position: pos(80, 130) },
            { type: 'NAND_2', label: 'NAND1', position: pos(200, 100) },
            { type: 'LED_YELLOW', label: 'LED1', position: pos(320, 100) },
        ],
        [
            ['SW1', 'NAND1:A'],
            ['SW2', 'NAND1:B'],
...
export const NOR_GATE_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_TOGGLE', label: 'SW1', position: pos(80, 70) },
            { type: 'SWITCH_TOGGLE', label: 'SW2', position: pos(80, 130) },
            { type: 'NOR_2', label: 'NOR1', position: pos(200, 100) },
            { type: 'LED_BLUE', label: 'LED1', position: pos(320, 100) },
        ],
        [
            ['SW1', 'NOR1:A'],
            ['SW2', 'NOR1:B'],
...
export const XOR_GATE_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_TOGGLE', label: 'SW1', position: pos(80, 70) },
            { type: 'SWITCH_TOGGLE', label: 'SW2', position: pos(80, 130) },
            { type: 'XOR_2', label: 'XOR1', position: pos(200, 100) },
            { type: 'LED_GREEN', label: 'LED1', position: pos(320, 100) },
        ],
        [
            ['SW1', 'XOR1:A'],
...
export const PUSH_BUTTON_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_PUSH', label: 'BTN1', position: pos(140, 100) },
            { type: 'LED_RED', label: 'LED1', position: pos(260, 100) },
        ],
        [['BTN1', 'LED1']]
    ),
...
export const CLOCK_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'CLOCK', label: 'CLK1', position: pos(140, 100) },
...
export const CONST_HIGH_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'CONST_HIGH', label: 'VCC', position: pos(140, 100) },
            { type: 'LED_RED', label: 'LED1', position: pos(260, 100) },
        ],
        [['VCC', 'LED1']]
    ),
...
export const CONST_LOW_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'CONST_LOW', label: 'GND', position: pos(140, 100) },
            { type: 'LED_RED', label: 'LED1', position: pos(260, 100) },
        ],
        [['GND', 'LED1']]
    ),
...
export const DISPLAY_7SEG_EXAMPLE: ExampleCircuit = {
    ...
    description: 'Press button to count 0-9',
    blueprint: circuit(
        [
            { type: 'SWITCH_PUSH', label: 'BTN', position: pos(60, 100) },
            { type: 'COUNTER_4BIT', label: 'CNT', position: pos(160, 100) },
            { type: 'BCD_TO_7SEG', label: 'DEC', position: pos(280, 100) },
            { type: 'DISPLAY_7SEG', label: 'DISP', position: pos(400, 100) },
        ],
        [
            ['BTN', 'CNT:CLK'],
            ['CNT:Q0', 'DEC:D0'],
...
export const D_FLIPFLOP_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_TOGGLE', label: 'DATA', position: pos(70, 70) },
            { type: 'SWITCH_PUSH', label: 'CLK', position: pos(70, 130) },
            { type: 'D_FLIPFLOP', label: 'DFF1', position: pos(200, 100) },
            { type: 'LED_GREEN', label: 'Q', position: pos(320, 80) },
            { type: 'LED_RED', label: 'QB', position: pos(320, 120) },
        ],
        [
            ['DATA', 'DFF1:D'],
            ['CLK', 'DFF1:CLK'],
...
export const SR_LATCH_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_PUSH', label: 'SET', position: pos(70, 70) },
            { type: 'SWITCH_PUSH', label: 'RST', position: pos(70, 130) },
            { type: 'SR_LATCH', label: 'SR1', position: pos(200, 100) },
            { type: 'LED_GREEN', label: 'Q', position: pos(320, 80) },
            { type: 'LED_RED', label: 'QB', position: pos(320, 120) },
        ],
        [
...
export const JK_FLIPFLOP_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_TOGGLE', label: 'J', position: pos(70, 55) },
            { type: 'SWITCH_PUSH', label: 'CLK', position: pos(70, 100) },
            { type: 'SWITCH_TOGGLE', label: 'K', position: pos(70, 145) },
            { type: 'JK_FLIPFLOP', label: 'JK1', position: pos(200, 100) },
            { type: 'LED_GREEN', label: 'Q', position: pos(320, 80) },
            { type: 'LED_RED', label: 'QB', position: pos(320, 120) },
        ],
        [
...
export const COUNTER_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'CLOCK', label: 'CLK', position: pos(80, 100) },
            { type: 'COUNTER_4BIT', label: 'CNT1', position: pos(190, 100) },
            { type: 'LED_RED', label: 'Q0', position: pos(310, 40) },
            { type: 'LED_RED', label: 'Q1', position: pos(310, 75) },
            { type: 'LED_RED', label: 'Q2', position: pos(310, 110) },
            { type: 'LED_RED', label: 'Q3', position: pos(310, 145) },
        ],
        [
            ['CLK', 'CNT1:CLK'],
            ['CNT1:Q0', 'Q0'],
...
export const MUX_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_TOGGLE', label: 'IN0', position: pos(70, 55) },
            { type: 'SWITCH_TOGGLE', label: 'IN1', position: pos(70, 100) },
            { type: 'SWITCH_TOGGLE', label: 'SEL', position: pos(70, 145) },
            { type: 'MUX_2TO1', label: 'MUX1', position: pos(200, 100) },
            { type: 'LED_GREEN', label: 'OUT', position: pos(320, 100) },
        ],
        [
            ['IN0', 'MUX1:A'],
            ['IN1', 'MUX1:B'],
...
export const DECODER_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_TOGGLE', label: 'A0', position: pos(70, 70) },
            { type: 'SWITCH_TOGGLE', label: 'A1', position: pos(70, 130) },
            { type: 'DECODER_2TO4', label: 'DEC1', position: pos(200, 100) },
            { type: 'LED_RED', label: 'Y0', position: pos(320, 40) },
            { type: 'LED_RED', label: 'Y1', position: pos(320, 75) },
            { type: 'LED_RED', label: 'Y2', position: pos(320, 110) },
            { type: 'LED_RED', label: 'Y3', position: pos(320, 145) },
        ],
        [
            ['A0', 'DEC1:A0'],
            ['A1', 'DEC1:A1'],
...
export const ADDER_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'SWITCH_TOGGLE', label: 'A0', position: pos(60, 40) },
            { type: 'SWITCH_TOGGLE', label: 'A1', position: pos(60, 75) },
            { type: 'SWITCH_TOGGLE', label: 'B0', position: pos(60, 120) },
            { type: 'SWITCH_TOGGLE', label: 'B1', position: pos(60, 155) },
            { type: 'ADDER_4BIT', label: 'ADD1', position: pos(200, 100) },
            { type: 'LED_GREEN', label: 'S0', position: pos(330, 60) },
            { type: 'LED_GREEN', label: 'S1', position: pos(330, 100) },
            { type: 'LED_RED', label: 'CO', position: pos(330, 140) },
        ],
        [
            ['A0', 'ADD1:A0'],
            ['A1', 'ADD1:A1'],
...
export const DIP_SWITCH_EXAMPLE: ExampleCircuit = {
    ...
    blueprint: circuit(
        [
            { type: 'DIP_SWITCH_4', label: 'DIP1', position: pos(140, 100) },
            { type: 'LED_RED', label: 'L0', position: pos(280, 45) },
            { type: 'LED_RED', label: 'L1', position: pos(280, 80) },
            { type: 'LED_RED', label: 'L2', position: pos(280, 115) },
            { type: 'LED_RED', label: 'L3', position: pos(280, 150) },
        ],
        [
            ['DIP1:Q0', 'L0'],
            ['DIP1:Q1', 'L1'],
...
export const EXAMPLE_CIRCUITS: Record<string, ExampleCircuit> = {
    ...
    BCD_TO_7SEG: DISPLAY_7SEG_EXAMPLE,
