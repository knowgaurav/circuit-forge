...
_TO_LEGACY = {Signal.HIGH: SignalState.HIGH, Signal.LOW: SignalState.LOW,
...
class SimulationService:
    ...
    INPUT_DEVICES = {ComponentType.SWITCH_TOGGLE, ComponentType.SWITCH_PUSH,
    ...
    def simulate(self, circuit: CircuitState) -> SimulationResult:
        ...
        if self._has_cycle(circuit):
            return SimulationResult(success=False, errors=[SimulationError(
                error_type="CYCLE_DETECTED",
                message="Circuit contains a cycle - cannot simulate")])
        engine = SimulationEngine()
        engine.load_circuit(circuit)
        pin_states: dict[str, dict[str, SignalState]] = {c.id: {} for c in circuit.components}
        ...
        wire_states = {wid: _TO_LEGACY[Signal(v)] for wid, v in engine.get_wire_states().items()}
        return SimulationResult(success=True, wire_states=wire_states, pin_states=pin_states)
